...
import numpy as np
import random 
#bfrom pygame.math import Vector2
import string
...
class Tile(pg.sprite.Sprite):
    ...
    def __init__(self
            ,x = 0
            ,y = 0
            ,img = os.getcwd() + '/Tiles/grassland.png'
            ,swimming = False
            ,landSpeed = 1
            ,vegValue = 0
            ):
        ...
        self.biome = self.img[len(os.getcwd()) + len("/Tiles/"):self.img.find(".png")]
...
class Desert(Tile):
    ...
    def __init__(self):
        ...
        self.landSpeed = 1.5
        self.img = os.getcwd() + '/Tiles/desert.png'
...
class Grassland(Tile):
    ...
    def __init__(self):
        ...
        self.landSpeed = 1.0
        self.img = os.getcwd() + '/Tiles/grassland.png'
        self.vegValue = 20
...
class Stone(Tile):
    ...
    def __init__(self):
        ...
        self.landSpeed = 1.0
        self.img = os.getcwd() + '/Tiles/stone.png'
        self.vegValue = 5
...
class Water(Tile):
    ...
    def __init__(self):
        ...
        self.swimming = True
        self.img = os.getcwd() + '/Tiles/water.png'
...
class Animal(pg.sprite.Sprite):
    def __init__(self
        ...
        self.refractoryRecovery = refractoryRecovery
        self.stamina = stamina
        self.needs = needs
        self.key = randomString(3)
    ...
    #updateHealth
    def update(self,area,oppCreatures, sameCreatures):
        self.updateLocationOrTarget(area, oppCreatures, sameCreatures)
        self.hungry(False)
        self.thirsty(area)
        self.ifDeath()
        self.display() 
    ...
    #set the target when the animal has environmental needs > reproduction or eating prey
    def targetTile(self,area):
        #Animal searches for nearest water tile
        if self.needs == "water":
            distance = 99999
            for col in area:
                for tile in col:
                    if tile.biome == "water":
                        checkDistance = math.sqrt((self.posx - tile.x) ** 2 + (self.posy - tile.y) ** 2)
                        if checkDistance <= distance:
                            distance = checkDistance
                            self.target = tile
        #Rabbits search for nearest grassland to travel to
        elif self.needs == "food":
            distance = 99999
            for col in area:
                for tile in col:
                    if tile.biome == "grassland":
                        ...
                        if checkDistance <= distance:
                            distance = checkDistance
                            self.target = tile
        verbose(self.species + " at (" + str(self.posx) + "," + str(self.posy) + ") is targetting tile at (" + str(self.target.x) + "," + str(self.target.y) +"), it is " + self.target.biome,True)

    #Track movement bonuses
    def updateLocationOrTarget(self,area, oppCreatures, sameCreatures):
        ...
        if self.prey == True:
            #Check if close to predator, if so creature is eaten
            for enemy in oppCreatures:
                if pg.sprite.collide_rect(self, enemy) == True:
                    self.health = 0
                    self.ifDeath()
                    enemy.hungry(True)
            #If priority is to reproduce then find closest
            verbose(self.species + "-" + self.key + "- at (" + str(self.posx) + "," + str(self.posy) + ") needs " + self.needs, False)
            if self.needs == "reproduce":
                distance = 99999
                for potentialMate in sameCreatures:
                    checkDistance = 99999
                    if self.key == potentialMate.key:
                        pass
                    elif pg.sprite.collide_rect(self,potentialMate) == True:
                        self.stamina = self.stamina - self.incubation
                        potentialMate.stamina = potentialMate.stamina - potentialMate.incubation
                        #sameCreatures.add(Rabbit()) #create new creature
                    elif checkDistance <= distance:
                        distance = checkDistance
                        self.target = potentialMate
                verbose(self.species + " at (" + str(self.posx) + "," + str(self.posy) + ") is targetting (" + str(self.target.posx) + "," + str(self.target.posy) + ") a " + self.target.species, False)
                    
            else:
                self.targetTile(area)
                verbose(self.species + " at (" + str(self.posx) + "," + str(self.posy) + ") is targetting (" + str(self.target.xpix) + "," + str(self.target.ypix) + ") a " + self.target.biome, False)
    ...
    #Priority
    def whatAreNeeds(self):
        h = self.hunger / self.full
        t = self.thirst / self.notThirst

        if h >= 0.8 and t >= 0.8:
            self.needs = "reproduce"
        elif t > h:
            self.needs = "food"
        else:
            self.needs = "water"

        verbose(self.species + " has " + str(h) + " food ratio and " + str(t) + " thirst ratio",False)
        verbose(self.species + " died. (" + str(self.posx) + "," + str(self.posy) + ")"
                + ", health = " + str(self.health)
                + ", hunger = " + str(self.hunger)
                + ", thirst = " + str(self.thirst), False)
        verbose("Priority of " + self.species + " at (" + str(self.posx) + "," + str(self.posy) + ") is " + self.needs, False)

    #Move toward target
    def moveTo(self, area):

        #Targetting Tile or Creature
        if self.target.img[len(os.getcwd())+1:len(os.getcwd())+6] == "Tiles":
            isTile = True
        else:
            isTile = False

        #Pick right destination coordinates
        if isTile:
            xgoal = self.target.xpix + int(tile_size / 2)
            ygoal = self.target.ypix + int(tile_size / 2)
        else:
            xgoal = self.target.posx
            ygoal = self.target.posy

        #Find angle to object
        try:
            A = abs(math.degrees(math.atan((self.posy-ygoal)/(self.posx-xgoal))))
        except ZeroDivisionError:
            A = 90

        verbose("Angle is " + str(A), False)
        verbose(self.species +" at (" + str(self.posx) + "," + str(self.posy) + ") is targeting (" + str(xgoal) + "," + str(ygoal) + ")", False)
...
def setAnimalSpawn(type, size, mult, tiles):
    ...
    while (i < N):
        ...
        verbose(creature.species + " initialized at tile (" + str(int(W / tile_size)+1) + "," + str(int(H / tile_size)+1) + ") type = " + (tiles[int(W / tile_size)][int(H / tile_size)]).biome , False)
        ...
        #while (tiles[int(W / tile_size)][int(H / tile_size)]).swimming == True and creature.swimSpeed == 0 :
...
def main():
    ...
    while not done:
        ...
        #Move the prey
        rabbits.update(map, predators, rabbits)

        #Move the predators
        predators.update(map, prey, foxes)
